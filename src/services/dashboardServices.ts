...
export const fetchDetailedProgress = async (
  token: string,
): Promise<DetailedProgressMap> => {
  const headers: ApiHeaders = {
    Authorization: `Bearer ${token}`,
  };

  const progressPromises = difficulties.map(async (difficulty) => {
    const response = await axios.get(`${API_BASE}/api/progress/${difficulty}`, {
      headers,
    });

    if (response.status < 200 || response.status >= 300) {
      throw new Error(
        `Progress fetch for ${difficulty} failed with status: ${response.status}`,
      );
    }
