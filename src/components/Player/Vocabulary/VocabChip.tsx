...
export const VocabChip: React.FC<VocabChipProps> = React.memo(
  ({ word, audioKey, onPlay, volume = 1 }) => {
    ...
    ${
      isPlaying
        ? "bg-gray-500/30 text-white ring-1 ring-gray-400/40"
        : "bg-transparent text-white/90 hover:bg-gray-500/15"
    }`}
        ...
        >
          <svg
            ...
            className={`w-3 h-3 flex-shrink-0 transition-transform duration-150 ${
              isPlaying ? "scale-110 animate-pulse text-green" : "text-green/80"
            }`}
          >
