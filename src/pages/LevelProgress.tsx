...
const LevelProgress = ({ 
  ...
  // Theme configurations for each difficulty level
  const themes = {
    easy: {
      title: 'ROOKIE',
      subtitle: 'Easy Level Progress',
    ...
    medium: {
      title: 'WARRIOR',
      subtitle: 'Medium Level Progress',
 ...
 const handleLevelClick = (level) => {
    if (level <= currentLevel) {
      // Передаем информацию о выбранном уровне и сложности
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="text-center mb-12">
          <div className="mb-4">
            ...
            <h1 className="text-4xl font-bold text-white mb-4">{theme.subtitle}</h1>
          </div>
              ...
              >
                ...
                {level === currentLevel && (
                  <div className="absolute -bottom-2 left-1/2 transform -translate-x-1/2">
                    <div className={`w-3 h-3 bg-gradient-to-r ${theme.currentGradient} rounded-full animate-bounce`}></div>
                  </div>
                )}
              </div>
            );
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-12">
          <div className="bg-white/10 backdrop-blur-sm rounded-xl p-6 text-center">
            <div className={`text-3xl font-bold ${theme.statColors.completed} mb-2`}>{completedLevels.length}</div>
            <div className="text-white/80">Levels Completed</div>
          </div>
          <div className="bg-white/10 backdrop-blur-sm rounded-xl p-6 text-center">
            <div className={`text-3xl font-bold ${theme.statColors.remaining} mb-2`}>{totalLevels - currentLevel + 1}</div>
            <div className="text-white/80">Levels Remaining</div>
          </div>
          <div className="bg-white/10 backdrop-blur-sm rounded-xl p-6 text-center">
            <div className={`text-3xl font-bold ${theme.statColors.progress} mb-2`}>{Math.round((completedLevels.length / totalLevels) * 100)}%</div>
