...
export async function signup(req, res) {
  try {
    ...
    const existingUser = await User.findOne({
      $or: [{ username }, { phoneNumber }].concat(email ? [{ email: email.toLowerCase() }] : []),
    });
    ...
    if (existingUser?.pendingRegistration) {
      await User.deleteOne({ _id: existingUser._id, pendingRegistration: true });
  ...
  try {
    ...
    if (req.session?.jti) {
      await User.updateOne(
        { _id: req.user._id },
        { $pull: { sessions: { jti: req.session.jti } } }
      );
    }
...
export async function verifyPhone(req, res) {
  try {
    ...
    const user = await User.findOne({
      phoneVerificationTicketHash: hash(ticket || ""),
      phoneVerificationTicketExpires: { $gt: new Date() },
    });
    if (!user) return res.status(400).json({ code: "INVALID_VERIFICATION", message: "Verification expired. Please request a new code." });
    ...
    if (user.phoneVerificationAttempts >= PHONE_MAX_ATTEMPTS) {
...
export async function startPhoneVerification(req, res) {
  try {
    ...
    const phoneNumber = normalizePhoneNumber(rawPhoneNumber);
    if (!usernameOrEmail || !password || !phoneNumber) {
    ...
    const phoneOwner = await User.findOne({ phoneNumber, _id: { $ne: user._id } }).select("_id");
    if (phoneOwner) return res.status(400).json({ message: "Phone number already exists" });
    const ticket = makeTicket();
